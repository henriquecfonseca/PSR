...
def calculate_accuracy(painted_image, zones, expected_colors):
    zone_accuracies = []
    ...
    for zone_number in range(1, np.max(zones) + 1):
        ...
        # Calculate the painted area in the expected color
        painted_area = np.all(painted_image == expected_color, axis=-1)

        # Resize painted_area to match the shape of zone_mask
        painted_area_resized = cv2.resize(painted_area.astype(np.uint8), (zone_mask.shape[1], zone_mask.shape[0]))

        # Calculate the percentage of overlap
        overlap_percentage = np.sum(np.logical_and(zone_mask, painted_area_resized)) / np.sum(zone_mask)

        zone_accuracies.append(overlap_percentage)
...
def main():
    ...
    if args['numered_paint']:
        ...
        accuracy = calculate_accuracy(tela, zones, expected_colors)
